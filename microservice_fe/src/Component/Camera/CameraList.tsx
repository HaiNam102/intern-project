...
const CameraList = (): JSX.Element => {
  ...
  const [cameraHealthStatus, setCameraHealthStatus] = useState<Record<number, CameraHealthData>>({});
  ...
  const handleViewStream = (camera: Camera): void => {
  ...
  const handleEditCamera = (camera: Camera): void => {
    navigate(`/camera/edit/${camera.cameraId}`, { 
      state: { camera } 
    });
  };

  const handleViewDetail = (camera: Camera): void => {
    navigate(`/camera/detail/${camera.cameraId}`);
  };

  const handleDeleteCamera = async (cameraId: number): Promise<void> => {
  ...
  const handleCheckHealth = async (cameraId: number): Promise<void> => {
    try {
  ...
  const handleCreateCamera = (): void => {
    navigate('/camera/create');
  };

  const getStatusColor = (
    status: string
  ): "success" | "error" | "warning" | "default" | "processing" | undefined => {
    switch (status) {
      case 'ONLINE':
        return 'success';
      case 'OFFLINE':
  ...
  const getRealTimeStatus = (camera: Camera): RealTimeStatus | null => {
  ...
  const getCameraImage = (cameraId: number): string => {
  ...
  const getCameraImageByStatus = (status: string): string => {
    switch (status) {
      case 'ONLINE':
                  ...
                  {realTimeStatus && (
                    <>
                      ...
                      <Descriptions.Item label="Cập nhật lúc">
                        <Tag color="purple">
                          {realTimeStatus.timestamp ? new Date(realTimeStatus.timestamp).toLocaleString('vi-VN') : 'N/A'}
                        </Tag>
                      </Descriptions.Item>
